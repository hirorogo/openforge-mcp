...
| `sync_game_studios` | Game Studios のエージェント定義をローカルまたは GitHub から同期 |

---

## エージェントの自動同期

Game Studios のエージェントが更新されると、OpenForge も自動的に追従します。`sync_game_studios` ツールを使うと、`.claude/agents/` ディレクトリ内のエージェント定義ファイル (*.md) をスキャンし、説明文のキーワードに基づいて OpenForge のツールカテゴリに自動マッピングします。

### ローカル同期

プロジェクトディレクトリから直接エージェントを読み取ります:

```
「Game Studios のエージェントをローカルから同期して」
```
...
`setup_game_studios` を呼ぶ際に `projectPath` を指定すると、ローカル同期も自動的に実行されます。

### GitHub 同期

ローカルにリポジトリがない場合、GitHub API から直接取得できます:

```
「Game Studios のエージェントを GitHub から同期して」
```
...
GitHub 同期の特徴:
- API レート制限を考慮し、結果を 1 時間キャッシュします
- 403/429 レスポンスを受けた場合はキャッシュ済みのデータを使い続けます
- ネットワークエラー時は静的なフォールバックが維持されます

### ローカル同期 vs GitHub 同期の違い

| | ローカル同期 | GitHub 同期 |
|--|-----------|-----------|
| 速度 | 高速 (ファイルシステム読み取り) | ネットワーク依存 |
| 前提条件 | リポジトリのローカルクローンが必要 | インターネット接続のみ |
| キャッシュ | なし (毎回最新を読み取り) | 1 時間キャッシュ |
| レート制限 | なし | GitHub API の制限あり |

どちらの場合も、静的に定義されたエージェント (lead-programmer, art-director 等) は常に利用可能です。動的に発見されたエージェントは静的定義に**追加**されます (置き換えではありません)。
