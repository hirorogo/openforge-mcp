...
export class ToolRouter {
  ...
  constructor(registry: ToolRegistry, unityAdapter: UnityAdapter, blenderAdapter: BlenderAdapter) {
  ...
  async execute(params: ExecuteParams): Promise<ExecuteResult> {
    ...
    if (target !== "unity" && target !== "blender") {
      return {
        success: false,
        target,
        tool: toolName,
        error: `Unknown target "${target}". Must be "unity" or "blender".`,
      };
    }
    ...
    const adapter = target === "unity" ? this.unityAdapter : this.blenderAdapter;
  ...
  getConnectionStatus(): { unity: boolean; blender: boolean } {
    return {
