...
describe("ToolRouter", () => {
  ...
  beforeEach(() => {
    ...
    router = new ToolRouter(registry, unityAdapter as any, blenderAdapter as any);
  });
  ...
  describe("routing to correct adapter", () => {
    ...
    it("should return the result data from the adapter response", async () => {
      unityAdapter.sendCommand.mockResolvedValueOnce({
        jsonrpc: "2.0",
        id: 1,
        result: { objectId: "abc123", name: "Cube" },
      });
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual({ objectId: "abc123", name: "Cube" });
    });
